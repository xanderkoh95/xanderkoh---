...
int Fast_Fourier_Transform(double *y_re, double *y_im, double *x_re, double *x_im, int N)
{
	...
	} else 
	{
		...
		*/
		...
		y_re[N/3+k]=y_30_re[k]+(-0.5)*(a+c);// problem in this one 
		y_im[N/3+k]=y_30_im[k]+(pow(3.0,0.5)/2)*(b-d);// problem in this one
		y_re[2*N/3+k]=y_30_re[k]+(-0.25)*(a+c);// problem in this one
		y_im[2*N/3+k]=-y_30_im[k]+pow((pow(3.0,0.5)/2),2)*(b+d);// problem in this one
