...
int Fast_Fourier_Transform(double *y_re, double *y_im, double *x_re, double *x_im, int N)
{
	double w1_re,w1_im,w2_re,w2_im,w3_re,w3_im,w4_re,w4_im;
	double w6_re,w6_im,w8_re,w8_im,w9_re,w9_im,w12_re,w12_im,w16_re,w16_im;
	double T;
	w1_re=0.3090169944;w1_im=0.9510565163;w2_re=-0.8090169944;w2_im=0.5877852523;w3_re=w2_re;w3_im=-w2_im;w4_re=w1_re;w4_im=-w1_im;
	w6_re=w1_re;w6_im=w1_im;w8_re=w2_re;w8_im=-w2_im;w9_re=w1_re;w9_im=-w1_im;w12_re=w2_re;w12_im=w2_im;w16_re=w1_re;w16_im=w1_im;
	clock_t t1, t2;
	if (N == 2)
	{
	...
	else if (N%3 == 0)
	{
		...
		double w_re, w_im, w_N_re, w_N_im, a, b , c , d , e ,temp ;
	   ...
	   //#pragma omp parallel for
		...
		c = w_re*w_re*y_32_re[k]-2*w_re*w_im*y_32_im[k]-w_im*w_im*y_32_re[k];  //real part
		d = w_re*w_re*y_32_im[k]+2*w_re*w_im*y_32_re[k]-w_im*w_im*y_32_im[k];
		e = (pow(3.0,0.5)/2);
		y_re[k]=y_30_re[k] + a + c;
		y_im[k]=y_30_im[k] + b + d;
		y_re[N/3+k]=y_30_re[k]-0.5*a+e*b-0.5*c-e*d;// problem in this one 
		y_im[N/3+k]=y_30_im[k]-0.5*b-e*a-0.5*d+e*c;// problem in this one
		y_re[2*N/3+k]=y_30_re[k]-0.5*a-e*b-0.5*c+e*d;// problem in this one
		y_im[2*N/3+k]=y_30_im[k]-0.5*b+e*a-0.5*d-e*c;// problem in this one
